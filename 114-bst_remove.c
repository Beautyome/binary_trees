...
bst_t *bst_remove_recursive(bst_t *root, bst_t *node, int val);
...
bst_t *bst_remove_recursive(bst_t *root, bst_t *node, int val)
{
	if (node != NULL)
	{
		...
		if (node->n > val)
			return (bst_remove_recursive(root, node->left, val));
		return (bst_remove_recursive(root, node->right, val));
	}
	return (NULL);
}
 ...
 * @root: A pointer to the root node of the BST to remove a node from.
 * @val: The value to remove in the BST.
 *
 * Return: A pointer to the new root node after deletion.
 *
...
bst_t *bst_remove(bst_t *root, int val)
{
	return (bst_remove_recursive(root, root, val));
}
